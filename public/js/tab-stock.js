...
<div class="sk-filters">
  <label>الفرع:</label>
  <div class="sk-chip-grp" id="sk-branch-chips">
    <div class="sk-chip active" data-branch="0">كل الفروع</div>
    <div class="sk-chip" data-branch="1">الفرع الرئيسي</div>
    <div class="sk-chip" data-branch="2">مصنع حوراء</div>
  </div>
  <label style="margin-right:10px">الفئة:</label>
...
`;
  ...
  // Branch chips
  wrap.querySelector('#sk-branch-chips').addEventListener('click', e => {
    const chip = e.target.closest('.sk-chip');
    if (!chip) return;
    wrap.querySelectorAll('#sk-branch-chips .sk-chip').forEach(c => c.classList.remove('active'));
    chip.classList.add('active');
    _skBranch = +chip.dataset.branch;
    _skRender();
  });
...
function _skFiltered() {
  if (!_skData) return [];
  let rows = _skData;
  if (_skBranch) rows = rows.filter(r => r.branch === _skBranch);
  if (_skCat !== 'all') rows = rows.filter(r => r.categoryName === _skCat);
  if (_skSearch) {
    const q = _skSearch.toLowerCase();
    rows = rows.filter(r => r.nameAr.toLowerCase().includes(q) || r.categoryName.toLowerCase().includes(q));
  }
  // Merge same item across branches when branch=all
  const byId = new Map();
  rows.forEach(r => {
    if (!byId.has(r.itemId)) byId.set(r.itemId, { ...r });
    else {
      const e = byId.get(r.itemId);
      e.qty   += r.qty;
      e.value += r.value;
      if (r.lastMovement > (e.lastMovement||'')) e.lastMovement = r.lastMovement;
    }
  });
  return [...byId.values()];
}
...
function _skRender() {
  ...
  const cols = [
    ...
    { key:'costPrice',    label:'سعر التكلفة (ر.س)', cls:'' },
    { key:'value',        label:'القيمة (ر.س)',       cls:'' },
    { key:'lastMovement', label:'آخر حركة',           cls:'' },
  ];
  ...
  if (!items.length) {
    html += `<tr><td colspan="7" class="sk-no-data">لا توجد أصناف تطابق الفلتر</td></tr>`;
  } else {
    items.forEach(r => {
      const barW = Math.round((r.value / maxVal) * 80);
      html += `<tr>
        ...
        <td class="num">${r.costPrice ? SK_FMT(r.costPrice) : '—'}</td>
        <td class="num hi"><span class="sk-val-bar" style="width:${barW}px"></span>${SK_FMT(r.value)}</td>
        <td style="color:#8a9bb5;font-size:.75rem">${r.lastMovement||'—'}</td>
      </tr>`;
    });
    // total row
...
function _skPrint(mode) {
  ...
  const BRANCH = { 0:'كل الفروع', 1:'الفرع الرئيسي', 2:'مصنع حوراء' };
  const company = State.get('companyName') || '';
  const dateStr = new Date().toLocaleDateString('ar-SA', { year:'numeric', month:'long', day:'numeric' });
  const el = id => document.getElementById(id);
  if (el('sk-ph-company')) el('sk-ph-company').textContent = company;
  if (el('sk-ph-filter'))  el('sk-ph-filter').textContent  = `الفرع: ${BRANCH[_skBranch]||'كل الفروع'} | الفئة: ${_skCat==='all'?'الكل':_skCat}`;
...
async function _skExportExcel() {
  ...
  const BRANCH  = { 0:'كل الفروع', 1:'الفرع الرئيسي', 2:'مصنع حوراء' };
  const branch  = BRANCH[_skBranch] || 'كل الفروع';
  const dateStr = new Date().toLocaleDateString('ar-SA', { year:'numeric', month:'long', day:'numeric' });
  const numFmt  = '#,##0.000';
  const valFmt  = '#,##0.00';
  ...
  ws.columns = [
    { width:42 }, { width:20 }, { width:14 }, { width:10 }, { width:16 }, { width:18 }, { width:14 },
  ];
  ...
  const r3 = ws.addRow([`الفرع: ${branch}  |  الفئة: ${_skCat==='all'?'الكل':_skCat}  |  ${dateStr}`,'','','','','','']);
  ...
  // Header row
  const hRow = ws.addRow(['اسم الصنف','الفئة','تاريخ آخر حركة','الوحدة','سعر التكلفة (ر.س)','القيمة (ر.س)','الكمية']);
  hRow.height = 22;
  ...
  catMap.forEach((list, cat) => {
    ...
    const gRow = ws.addRow([`📦  ${cat}  (${list.length} صنف)`,'','','',''  ,catVal, catQty]);
    ws.mergeCells(`A${gRow.number}:E${gRow.number}`);
    gRow.height = 20;
    gRow.eachCell((c,i) => { c.fill=grpFill; c.font=grpFont; c.alignment=i<=5?rtl:num; if(i>=6)c.numFmt=valFmt; });

    list.forEach((r,idx) => {
      const dRow = ws.addRow([r.nameAr, r.categoryName, r.lastMovement||'—', r.unitName, r.costPrice||0, r.value, r.qty]);
      dRow.height = 17;
      ...
      [1,2,3,4,5,6,7].forEach(i => {
        ...
        c.alignment = i<=2 ? rtl : i===3 ? cen : num;
        if (i>=5) c.numFmt = valFmt;
        if (i===7) c.numFmt = numFmt;
        if (i===6) c.font = { name:FONT, size:10, bold:true, color:{ argb:'FF1a6a3c' } };
      });
    });

    const sRow = ws.addRow([`إجمالي ${cat}`,'','','','',catVal,catQty]);
    ws.mergeCells(`A${sRow.number}:E${sRow.number}`);
    sRow.height = 18;
    sRow.eachCell((c,i)=>{ c.fill=totFill; c.font=totFont; c.alignment=i<=5?rtl:num; if(i>=6)c.numFmt=valFmt; c.border={top:{style:'thin',color:{argb:CLR.navyMid}}}; });
    ws.addRow([]);
  });

  // Grand total
  const gt = ws.addRow(['الإجمالي الكلي','','','','',grandVal,grandQty]);
  ws.mergeCells(`A${gt.number}:E${gt.number}`);
  gt.height = 24;
  gt.eachCell((c,i)=>{ c.fill=grandFill; c.font=grandFont; c.alignment=i<=5?rtl:num; if(i>=6)c.numFmt=valFmt; c.border={top:{style:'medium',color:{argb:CLR.green}},bottom:{style:'medium',color:{argb:CLR.green}}}; });
  ...
  try {
    ...
    a.href = url;
    a.download = `رصيد_المخزون_${branch}_${new Date().toISOString().slice(0,10)}.xlsx`;
    a.click();
